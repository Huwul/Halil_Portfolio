...
export const Blog = () => {
    ...
    useEffect(() => {
        ...
        } else if (isClientSideFiltering && !selectedTag) {
    ...
    }, [searchTerm, allPosts, currentPage, isClientSideFiltering, selectedTag]);

    // Sadece tag filtreleme ve ilk yükleme için API çağrısı
    useEffect(() => {
        if (searchTerm || (isClientSideFiltering && !selectedTag)) return; // Search aktifse veya search temizlendiyse API çağırma

        const fetchPosts = async () => {
            try {
                // Sadece ilk yüklemede loading göster
                if (initialLoading) {
                    // Initial loading zaten true, bu yüzden setInitialLoading(true) gerekmiyor
                }
                let response: BlogResponse;

                if (selectedTag) {
                    response = await blogService.getPostsByTag(
                        selectedTag,
                        currentPage,
                        6
                    );
    ...
    const handleTagClick = (tag: string) => {
        ...
        setIsClientSideFiltering(false); // Tag seçildiğinde client-side filtreleme durumunu reset et
    };
    ...
    const handleSearchChange = (value: string) => {
        ...
        setCurrentPage(1);
        if (!value) {
            setIsClientSideFiltering(false); // Search temizlendiğinde reset et
        }
    };

    if (initialLoading && !searchTerm && !isClientSideFiltering) {
        return (
            ...
            <section className="section-padding bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
                <div className="max-w-7xl mx-auto text-center">
                    ...
                    <p className="text-xl text-gray-300 dark:text-gray-300 max-w-3xl mx-auto mb-8">
                        Thoughts, ideas, and insights about software
                        development, technology, and the journey of building
                        ...
                        <>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                {posts.map((post) => (
                                    <article
                                        key={post._id}
                                        className="card hover:shadow-xl transition-shadow duration-300 h-full flex flex-col"
                                    >
                                        {post.featuredImage && (
